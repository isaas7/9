...
session::session(tcp::socket &&socket, std::shared_ptr<ChatService> chatService)
    : stream_(std::move(socket)), chatService_(chatService) {}
...
void session::on_read(beast::error_code ec, std::size_t bytes_transferred) {
  ...
  if (ec)
    return fail(ec, "read");
  send_response(handle_request(std::move(req_), chatService_));
}
